...
export class SellComponent implements OnInit {
  ...
  init() {
    this.goodsService.getApp().then(res => {
      this.apps = res.json().result.data;
  ...
  tempQuery(appcode?: number) {
    ...
    this.goodsService.getTemplate([], appcode).then(res => {
  ...
  delChannel(t: number, c: number) {
    this.goodsService.delChannel(c, t).then(res => {
      console.log(res.json());
      this.init();
      this.message.info('分类已删除');
    });
  }

  addGoods(c: any) {
    const tmp = [];
    this.goodsService.getAllGoods(this.i).then(res => {
  ...
  delGoods(c: number, g: number) {
    this.goodsService.delChannelBand(c, g).then(res => {
      console.log(res.json());
      this.init();
      this.message.info('商品已移除');
    });
  }
}
