...
export class SellComponent implements OnInit {
  ...
  confirm() {
    this.goodsService.createTemplate()
          .then(res => {
  ...
  bandTemplateAPPID(t: number) {
    this.message.info('请联系管理员罗烽绑定');
    /* this.goodsService.bindingTA(t, 'test')
          .then(res => {
            console.log(res.json());
            this.init();
  ...
  newChannel(t: number) {
    ...
    subscription.subscribe((result: any) => {
      console.log(result);
      if (result.name) {
        this.goodsService.createChannel(result)
              .then(res => {
                console.log(res.json());
                this.init();
  ...
  editChannel(t: number, c: any) {
    ...
    subscription.subscribe((result: any) => {
      console.log(result);
      if (result.name) {
        this.goodsService.updateChannel(result)
              .then(res => {
                console.log(res.json());
                this.init();
                this._notification.create(`success`, '操作提醒', '修改分裂成功');
                subscription.destroy(); // TODO ??
              });
      }
    });
  }
  addGoods(c: any) {
    ...
    this.goodsService.getAllGoods()
          ...
          subscription.subscribe((result: any) => {
            console.log(result);
            if (result.channel) {
              this.goodsService.bandChannel(result, 'band')
                    .then(res => {
                      console.log(res.json());
                      this.init();
